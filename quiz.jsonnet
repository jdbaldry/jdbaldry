...

{
  ...
  local palindromes = round.new('Palindromes', 'Every answer is a palindrome, spelled the same forwards as backwards. For example, "racecar".').withQuestions([
    ...
    question.new('What is the first name of the Disney character played by Miley Cyrus?', 'Hannah'),
    question.new('Complete the phrase used to describe a competition between men and women, _Battle of the \\_\\_\\_\\_\\__', 'sexes'),
  ]),
  local pictures = round.new('Pictures', 'Guess the TV show from the picture').withQuestions([
    ...
    question.new('![](https://imagesvc.meredithcorp.io/v3/mm/image?url=https%3A%2F%2Fstatic.onecms.io%2Fwp-content%2Fuploads%2Fsites%2F6%2F2017%2F03%2F138425365-2000.jpg)', 'Fresh Prince of Bel Air'),
  ...
  local songs = round.new('Songs', 'Guess the song from the first two lines').withQuestions([
    ...
    question.new('Chestnuts roasting on an open fire.\n\nJack Frost nipping at your nose.', 'The Christmas Song, Nat King Cole'),
  ...
  local specialist = round.new('Specialist', "Minus one point if you don't get your specialist question right!").withQuestions([
    question.new('Archie:\n\nWhich company has produced three of the top five best selling video game consoles of all time?', 'Sony'),
    question.new("Mum:\n\nWhich all girl group from the 1980's had number ones with _Walk like an Egyptian_ and _Eternal Flame_?", 'The Bangles'),
    question.new("David:\n\nWhich soul singer had a smash hit album called _Let's Stay Together_", 'Al Green'),
    question.new('Nathan:\n\nWho was the scorer of the winning goal in the 1972 FA Cup final win over Arsenal', 'Allan Clarke'),
    question.new('Emma:\n\nWhat was the first ever Disney feature length film?', 'Snow White and the Seven Dwarves'),
    question.new('Molly:\n\nWhat type of animal is _Timmy Tiptoes_?', 'Squirrel'),
    question.new('Ross:\n\nAccording to _RSBPs Big Garden Bird Watch_, what is the most common garden bird?', 'Sparrow'),
  ]),
  ...
  'index.md': q.withRounds([dingbats, palindromes, pictures, general, songs, specialist]).renderMarkdown(),
}
